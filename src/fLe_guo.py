...
class fle_guo():
    ...
    def params(self, T, h = 0.01, v0 = 1, eta = 1, m = 1, KBT = 1):
        ...
        H = self.H
        self.zeta = np.sqrt((KBT*eta) * gamma(1.5 - H) * gamma(0.5 + H) / ((2*H - 1) * gamma(2 - 2*H)))
        # wrong expression in paper, gamma(0.5 - 1) < 0: 
        # self.zeta = np.sqrt((KBT*eta) * gamma(0.5 - H) * gamma(0.5 + H) / ((2*H - 1) * gamma(2 - 2*H))) 
    ...
    def get_analytical(self):
        ...
        self.analytical = nonlinear + (zeta) * conv
    ...
    def get_msd_analytical_limit(self):
        ...
        order = 2 - 2*H
        #*gamma(1-order)
        self.msd_analytical_limit = 2*KBT/(m*eta) * (t**(2 - 2*H))/(gamma(2*H - 1)*gamma(3 - 2*H))